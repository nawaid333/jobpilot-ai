...
test("Agent follow-up is scheduled exactly three days ahead", () => {
  ...
  const due = nextFollowUpDate(now);
  assert.equal(due.toISOString(), "2026-09-16T12:00:00.000Z");
  assert.equal(due.getTime() - new Date("2026-09-13T12:00:00Z").getTime(), 3 * DAY);
});

test("an existing follow-up remains the source of truth", () => {
  const existing = new Date("2026-09-20T12:00:00Z");
  assert.equal(existing.toISOString(), "2026-09-20T12:00:00.000Z");
});
