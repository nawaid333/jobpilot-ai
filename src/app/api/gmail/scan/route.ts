...
function safeDate(value:string){const date=value?new Date(value):null;return date&&!Number.isNaN(date.getTime())?date:null}
...
export async function POST(){
  ...
  try{
    ...
    let scanned=0,signals=0,matched=0,aiMatched=0,ambiguous=0,failed=0;
    let pageToken="";
    do{
      const query=`/users/me/messages?maxResults=50&q=newer_than:30d+(application+OR+interview+OR+assessment+OR+offer+OR+recruiter)${pageToken?`&pageToken=${encodeURIComponent(pageToken)}`:""}`;
      const list=await gmailGet(query,access.access_token);
      for(const item of (list.messages||[])){
        try{
          ...
          try{
            await prisma.emailSignal.create({data:{userId:user.id,gmailMessageId:item.id,threadId:message.threadId,sender,subject,receivedAt:safeDate(received),category:signal.category,confidence:signal.confidence,suggestedStatus:signal.suggestedStatus,reason:explanation,applicationId,jobId,recruiterName,recruiterEmail,matchedScore,matchMethod,ambiguous:isAmbiguous}});
            signals++;
          }catch(error){
            if(!(error instanceof Error&&error.message.includes("Unique constraint")))throw error;
          }
          scanned++;
        }catch{failed++;scanned++;}
      }
      pageToken=typeof list.nextPageToken==="string"?list.nextPageToken:"";
    }while(pageToken);
    return NextResponse.json({ok:true,scanned,signals,matched,aiMatched,ambiguous,failed});
