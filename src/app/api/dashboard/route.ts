...
export async function GET(){
 ...
 try {
  const applications=await prisma.application.findMany({where:{userId:user.id},select:{id:true,status:true,updatedAt:true,job:{select:{title:true,company:true}},emailSignals:{orderBy:{receivedAt:"desc"},take:1,select:{receivedAt:true,category:true,recruiterName:true}}},orderBy:{updatedAt:"desc"},take:100});
  ...
  return NextResponse.json({error:"Dashboard is temporarily unavailable."},{status:503,headers:{"Cache-Control":"no-store, max-age=0"}});
 }
}
