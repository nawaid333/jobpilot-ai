...
export async function GET(){
 ...
 const applications=await prisma.application.findMany({where:{userId:user.id},include:{job:true,emailSignals:{orderBy:{receivedAt:"desc"},take:3}}});
 ...
 return NextResponse.json({summary:{total:applications.length,applied:counts.Applied||0,interviews:counts.Interview||0,offers:counts.Offer||0,rejected:counts.Rejected||0,saved:counts.Saved||0,preparing:counts.Preparing||0},actions});
}
