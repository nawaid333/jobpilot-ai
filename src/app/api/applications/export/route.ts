...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (experience.length) { heading("Professional Experience"); for (const item of experience) { const title = [clean(item.role), clean(item.company)].filter(Boolean).join(" — "); if (title) children.push(new Paragraph({ children: [new TextRun({ text: title, bold: true })], spacing: { before: 60, after: 15 } })); if (clean(item.duration)) children.push(new Paragraph({ text: clean(item.duration), italics: true, spacing: { after: 40 } })); for (const h of list(item.highlights)) bullet(h); } }
    if (skills.length) { heading("Core Skills"); children.push(new Paragraph({ text: skills.join("  •  ") })); }
    if (education.length) { heading("Education"); for (const item of education) { const title = [clean(item.degree), clean(item.institution)].filter(Boolean).join(" — "); if (title) children.push(new Paragraph({ children: [new TextRun({ text: title, bold: true })], spacing: { after: 15 } })); if (clean(item.year)) children.push(new Paragraph({ text: clean(item.year), italics: true, spacing: { after: 35 } })); } }
    ...
    children.push(new Paragraph({ text: `Prepared for ${clean(app.job.title) || "target role"} at ${clean(app.job.company) || "target company"}. JobPilot uses only information present in the saved candidate profile. Review before submitting.`, spacing: { before: 180 }, italics: true }));
