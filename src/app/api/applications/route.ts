...
function originViolation(req: Request) {
  ...
  try { const expected = new URL(process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000").origin; return new URL(origin).origin !== expected; } catch { return true; }
}
...
export async function DELETE(req: Request) {
  const user = await getCurrentUser(); if (!user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 }); const limited = guard(user.id, "write"); if (limited) return limited;
  if (originViolation(req)) return NextResponse.json({ error: "Invalid request origin." }, { status: 403 }); if (bodyTooLarge(req)) return NextResponse.json({ error: "Request body is too large." }, { status: 413 }); const b = await req.json().catch(() => ({})); if (!b || typeof b !== "object" || !b.id) return NextResponse.json({ error: "Application id is required." }, { status: 400 });
  try { const id = stringField(b.id, 300, "Application id"); const result = await prisma.application.deleteMany({ where: { id, userId: user.id } }); if (!result.count) return NextResponse.json({ error: "Application not found." }, { status: 404 }); return NextResponse.json({ ok: true }, { headers: { "Cache-Control": "private, no-store, max-age=0" } }); } catch { return NextResponse.json({ error: "Could not delete application." }, { status: 400 }); }
}
