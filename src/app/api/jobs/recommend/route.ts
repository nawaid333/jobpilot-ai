...
export async function GET() {
  ...
  if (live.jobs.length) {
    await prisma.$transaction(live.jobs.map(job => prisma.job.upsert({
      where: { id: job.id },
      create: job,
  ...
  return NextResponse.json({ recommendations, generatedAt: new Date().toISOString(), strategy: "live-lever-profile-preferences-evidence", configured: live.configured, sourceCount: live.jobs.length });
}
