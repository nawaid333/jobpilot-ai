...
export async function GET(){
 ...
 const [profile,preferences,applications,live,storedJobs]=await Promise.all([prisma.careerProfile.findUnique({where:{userId:user.id}}),prisma.jobPreferences.findUnique({where:{userId:user.id}}),prisma.application.findMany({where:{userId:user.id},select:{jobId:true}}),fetchLiveJobs(),prisma.job.findMany({orderBy:{lastSeenAt:"desc"},take:100})]);
 ...
 const candidateJobs=live.jobs.length?live.jobs:storedJobs.map(job=>({id:job.id,title:job.title,company:job.company,location:job.location,mode:job.mode||"Not specified",level:job.level||"Not specified",salary:job.salary||"",url:job.url||"",description:job.description||"",skills:jsonStrings(job.skills),source:job.source||"Stored"}));
 const saved=new Set(applications.map(a=>a.jobId));
 const recommendations=candidateJobs.filter(job=>!saved.has(job.id)).map(job=>({job,result:score(job,profile,preferences)})).filter(x=>x.result.score>=55).sort((a,b)=>b.result.score-a.result.score).slice(0,20).map(x=>({...x.job,matchScore:x.result.score,matchReasons:x.result.reasons,matchedRoles:x.result.matchedRoles,matchedSkills:x.result.matchedSkills,scoreBreakdown:x.result.breakdown}));
 return NextResponse.json({recommendations,generatedAt:new Date().toISOString(),strategy:"multi-source-live-with-stored-fallback-explainable-role-skill-preference-evidence",configured:live.configured,sourceCount:live.sourceCount,connectedSources:live.sourceCount,usedStoredFallback:live.jobs.length===0&&storedJobs.length>0});
}
