  ...
  try {
    ...
    if (action === "prepare") {
      ...
      if (FINAL_STATUSES.has(application.status)) {
        return NextResponse.json({ error: `Cannot prepare an application already marked ${application.status}.` }, { status: 409 });
      }
      if (RANK[application.status] < RANK.Preparing) {
        await prisma.application.update({ where: { id: application.id }, data: { status: "Preparing" } });
      }
    ...
    if (action === "mark-preparing") {
      if (FINAL_STATUSES.has(application.status) || application.status === "Applied") {
        return NextResponse.json({ error: `Cannot move a ${application.status} application back to Preparing.` }, { status: 409 });
      }
      if (application.status === "Preparing") {
        return NextResponse.json({ ok: true, status: application.status, alreadyRecorded: true, message: "Application is already in Preparing." });
      }
    ...
    if (FINAL_STATUSES.has(application.status)) {
      return NextResponse.json({ error: `Cannot mark a ${application.status} application as Applied.` }, { status: 409 });
    }
    if (application.status === "Applied") {
      return NextResponse.json({ ok: true, status: application.status, alreadyRecorded: true, message: "Application is already marked as Applied." });
    }
