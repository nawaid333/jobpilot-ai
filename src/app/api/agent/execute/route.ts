...
function nextFollowUpDate(from = new Date()) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!applicationId || applicationId.length > 100 || !safeActions.has(action)) {
    ...
    if (action === "prepare") {
      if (!application.tailoredApplication) response = { ok: false, next: "tailor", redirect: `/tailor?applicationId=${id}`, message: "Create the tailored package before preparing submission." };
        ...
        response = { ok: true, next: "review", redirect: `/automation?applicationId=${id}`, message: "Your tailored package is ready for review." };
      }
      ...
      if (FINAL_STATUSES.has(application.status) || application.status === "Applied") response = { error: `Cannot move a ${application.status} application back to Preparing.`, status: 409 };
        ...
        response = { ok: true, status: updated.status, message: "Application moved to Preparing." };
      }
      ...
      if (FINAL_STATUSES.has(application.status)) response = { error: `Cannot mark a ${application.status} application as Applied.`, status: 409 };
        ...
        response = { ok: true, status: updated.status, message: "Marked as Applied. JobPilot did not submit the application." };
      }
    } else if (action === "follow-up") {
      if (FINAL_STATUSES.has(application.status)) {
        response = { error: `Cannot schedule Agent follow-up for a ${application.status} application.`, status: 409 };
      } else if (application.followUpDueAt) {
        response = {
          ok: true,
          next: "follow-up",
          redirect: `/application/${id}`,
          followUpDueAt: application.followUpDueAt.toISOString(),
          message: "A follow-up is already scheduled. Open the application to review it.",
        };
      } else {
        const dueAt = nextFollowUpDate();
        await prisma.application.update({ where: { id: application.id }, data: { followUpDueAt: dueAt } });
        response = {
          ok: true,
          next: "follow-up",
          redirect: `/application/${id}`,
          followUpDueAt: dueAt.toISOString(),
          message: `Follow-up scheduled for ${dueAt.toLocaleDateString()}. Review the application before contacting the recruiter.`,
        };
      }
    ...
    if (response.ok && !response.alreadyRecorded) {
      await record(user.id, application.id, action, "completed", {
        ...
        followUpDueAt: response.followUpDueAt || null,
