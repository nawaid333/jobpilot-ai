...
function nextFollowUpDate(from = new Date(), days = FOLLOW_UP_DELAY_DAYS) { const due = new Date(from); due.setDate(due.getDate() + days); due.setHours(12, 0, 0, 0); return due; }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (action === "prepare") {
      ...
      else { const dueAt = nextFollowUpDate(); await prisma.application.update({ where: { id: application.id }, data: { followUpDueAt: dueAt } }); response = { ok: true, next: "follow-up", redirect: `/application/${id}`, followUpDueAt: dueAt.toISOString(), message: `Follow-up scheduled for ${dueAt.toLocaleDateString()}. Review the application before contacting the recruiter.` }; }
      ...
      if (!application.followUpDueAt) response = { error: "No scheduled follow-up exists for this application.", status: 409 };
      else { const dueAt = nextFollowUpDate(new Date(), FOLLOW_UP_DELAY_DAYS); await prisma.application.update({ where: { id: application.id }, data: { followUpDueAt: dueAt } }); response = { ok: true, next: "follow-up", followUpDueAt: dueAt.toISOString(), message: `Follow-up snoozed until ${dueAt.toLocaleDateString()}.` }; }
    ...
    if (response.ok && !response.alreadyRecorded) await record(user.id, application.id, action, "completed", { next: response.next || null, status: response.status || application.status, followUpDueAt: response.followUpDueAt || null, message: response.message || null });
