...
export async function POST(req: Request) {
  ...
  if (mode === "feedback") {
    ...
    try {
      ...
      return NextResponse.json({ feedback: JSON.parse(data.choices?.[0]?.message?.content || "{}"), mode: "ai", remainingAi: credit.entitlements.remainingAi });
    } catch {
      return NextResponse.json({ feedback: rulesFeedback(), mode: "rules", remainingAi: credit.entitlements.remainingAi });
    }
  }
  ...
  try {
    ...
    return NextResponse.json({ questions: fallback, mode: "rules", job: application.job, remainingAi: credit.entitlements.remainingAi });
  }
}
