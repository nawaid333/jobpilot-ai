...
export async function POST(req: Request) {
  ...
  if (!process.env.OPENAI_API_KEY) {
    if (mode === "feedback") return NextResponse.json({ feedback: { score: null, verdict: "Evidence review", strengths: ["You answered the question directly."], improvements: ["Add a specific situation, your actions, and a measurable or observable result where truthful."], followUp: "What was your specific contribution and what changed because of it?" }, mode: "rules" });
    return NextResponse.json({ questions: fallback, mode: "rules", job: application.job });
  }
  const credit = await consumeAiCredit(user.id);
  if (!credit.ok) return NextResponse.json({ error: "You have reached your monthly AI limit. Interview practice is still available in evidence-rules mode.", code: "AI_LIMIT", entitlements: credit.entitlements }, { status: 429 });
  ...
  if (mode === "feedback") {
    ...
    try {
      const response = await fetch("https://api.openai.com/v1/chat/completions", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${process.env.OPENAI_API_KEY}` }, body: JSON.stringify({ model: "gpt-5.6-luna", messages: [{ role: "user", content: prompt }], temperature: 0.2, response_format: { type: "json_object" } }) });
      if (!response.ok) throw new Error("AI request failed");
      const data = await response.json(); return NextResponse.json({ feedback: JSON.parse(data.choices?.[0]?.message?.content || "{}"), mode: "ai", remainingAi: credit.entitlements.remainingAi });
    } catch { return NextResponse.json({ feedback: { score: null, verdict: "Evidence review", strengths: ["Your answer can be strengthened with concrete evidence."], improvements: ["Use Situation → Action → Result and only include facts you can support."], followUp: "What exactly did you do, and what was the result?" }, mode: "rules", remainingAi: credit.entitlements.remainingAi }); }
  }
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${process.env.OPENAI_API_KEY}` }, body: JSON.stringify({ model: "gpt-5.6-luna", messages: [{ role: "user", content: prompt }], temperature: 0.3, response_format: { type: "json_object" } }) });
    if (!response.ok) throw new Error("OpenAI request failed");
    const data = await response.json(); const parsed = JSON.parse(data.choices?.[0]?.message?.content || "{}"); const questions = Array.isArray(parsed.questions) ? parsed.questions.slice(0,8) : fallback;
    return NextResponse.json({ questions, mode: "ai", job: application.job, remainingAi: credit.entitlements.remainingAi });
  } catch { return NextResponse.json({ questions: fallback, mode: "rules", job: application.job, remainingAi: credit.entitlements.remainingAi }); }
}
