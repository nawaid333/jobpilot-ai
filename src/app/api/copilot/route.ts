...
export async function GET(){
 ...
 const usage=await consumeAiCredit(user.id);
 if(!usage.ok){const entitlements=await getEntitlements(user.id);return NextResponse.json({recommendations:[],mode:"limit",error:"Monthly AI limit reached.",plan:entitlements.planKey,usage:entitlements.usage,remainingAi:0},{status:429});}
