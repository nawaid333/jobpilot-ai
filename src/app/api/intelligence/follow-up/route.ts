...
export async function POST(req:Request){
  ...
  try{
    ...
    if(!key)return NextResponse.json({draft:fallback(app.job,signal?.recruiterName||null),source:"template"});
    ...
    return NextResponse.json({draft:{subject:parsed.subject.slice(0,180),body:parsed.body.slice(0,5000)},source:"ai"});
