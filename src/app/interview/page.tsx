...
export default function InterviewPage() {
  ...
  async function start() {
    ...
    try {
      ...
      setQuestions(data.questions || []); setMode(data.mode);
  ...
  async function review() {
    ...
    try {
      ...
      setFeedback(data.feedback); setMode(data.mode);
    } catch (e: any) { setError(e.message); }
    finally { setLoading(false); }
  }
    ...
    <section className="interview-shell shell">
      ...
      <div className="interview-grid">
        <aside className="interview-sidebar">
          ...
          <select value={applicationId} onChange={e => { setApplicationId(e.target.value); setQuestions([]); setFeedback(null); }}>
        ...
        <section className="interview-main">
          {!q ? <div className="interview-empty"><div className="kicker">PRACTICE MODE</div><h2>Choose a role to start.</h2><p>You'll get a focused question set, then answer one question at a time and receive evidence-based coaching.</p></div> : <>
            ...
            <div className="answer-actions"><button className="button primary" disabled={!answer.trim() || loading} onClick={review}>{loading ? "Reviewing…" : "Get AI feedback →"}</button>{feedback && index < questions.length - 1 && <button className="button secondary" onClick={next}>Next question →</button>}</div>
