...
export default function ApplicationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => { if (!id) return; fetch(`/api/applications/${encodeURIComponent(id)}`).then(r => r.json()).then(d => { if (d.application) { setApplication(d.application); setNotes(d.application.notes || ""); } else setError(d.error || "Application not found."); }).catch(() => setError("Could not load application.")).finally(() => setLoading(false)); }, [id]);
  ...
  async function saveNotes() {
    if (!id) return;
    setSavingNotes(true); setError(""); setNotesMessage("");
    try {
      const res = await fetch("/api/applications", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, notes }) });
      const data = await res.json();
  ...
  return <main className="page-shell">
    ...
    <section className="panel"><div className="section-heading"><div><p className="eyebrow">APPLICATION NOTES</p><h2>Your private context for this application</h2></div></div><textarea value={notes} onChange={e => { setNotes(e.target.value); setNotesMessage(""); }} rows={7} maxLength={10000} placeholder="Add recruiter names, interview notes, salary details, follow-up reminders, or anything you want to remember…"/><div className="profile-actions"><button className="primary-button" onClick={saveNotes} disabled={savingNotes}>{savingNotes ? "Saving…" : "Save notes"}</button>{notesMessage && <span className="source-notice">{notesMessage}</span>}</div></section>
