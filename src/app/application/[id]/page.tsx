...
export default async function ApplicationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const application = await prisma.application.findFirst({
    ...
    include: {
      ...
      agentActions: { where: { status: "pending" }, orderBy: [{ priority: "desc" }, { createdAt: "desc" }], take: 5 },
    },
  });
  ...
  return (
    <main className="page-shell">
      ...
      <section className="stats-grid"><div className="stat-card"><span>Applied</span><strong>{applied}</strong></div><div className="stat-card"><span>Next action</span><strong>{nextAction}</strong></div><div className="stat-card"><span>Recruiter signals</span><strong>{application.emailSignals.length}</strong></div><div className="stat-card"><span>Pending actions</span><strong>{application.agentActions.length}</strong></div></section>
