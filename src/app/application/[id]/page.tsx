...
export default async function ApplicationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const application = await prisma.application.findFirst({ where: { id, userId: user.id }, include: { job: true, tailoredApplication: true, emailSignals: { orderBy: { receivedAt: "desc" }, take: 8 }, agentActions: { orderBy: [{ priority: "desc" }, { createdAt: "desc" }], take: 8 } } });
  ...
  const readinessChecks = [
    { label: "Career profile", ready: true },
    { label: "Job saved", ready: !!application.job },
    { label: "Tailored package", ready: !!application.tailoredApplication },
    { label: "Materials reviewed", ready: !!application.tailoredApplication },
    { label: "Submitted", ready: ["Applied", "Interview", "Offer", "Rejected"].includes(application.status) },
  ];
  ...
  return <main className="page-shell">
    ...
    <section className="panel"><div className="section-heading"><div><p className="eyebrow">APPLICATION READINESS</p><h2>Ready to move forward?</h2></div><strong>{readiness}%</strong></div><div className="readiness-meter"><div style={{ width: `${readiness}%` }} /></div><div className="readiness-grid">{readinessChecks.map(check => <div className="readiness-item" key={check.label}><span>{check.ready ? "✓" : "○"}</span><strong>{check.label}</strong></div>)}</div>{!application.tailoredApplication && <p className="truth-note">Generate a tailored package before submitting so you can review role-specific materials.</p>}</section>
    <div className="dashboard-grid"><section className="panel"><p className="eyebrow">APPLICATION PLAN</p><h2>Move this application forward.</h2><div className="profile-actions"><Link className="primary-button" href={`/tailor?applicationId=${encodeURIComponent(application.id)}`}>Open tailoring</Link><Link className="secondary-button" href={`/intelligence?applicationId=${encodeURIComponent(application.id)}`}>Review signals</Link><Link className="secondary-button" href="/agent">Open Agent</Link></div><p className="truth-note">Review every generated document and recruiting signal before sending or submitting anything.</p></section><section className="panel"><p className="eyebrow">TAILORED PACKAGE</p><h2>{application.tailoredApplication ? "Ready for review" : "Not prepared yet"}</h2>{application.tailoredApplication ? <><p>{application.tailoredApplication.recommendation}</p><p className="muted">Template: {application.tailoredApplication.template} · Updated {new Date(application.tailoredApplication.updatedAt).toLocaleDateString()}</p></> : <p className="muted">Build a role-specific package from your saved career profile.</p>}</section></div>
