      ...
      {!analysis ? (
        ...
        <section className="results-shell shell">
          ...
          <div className="result-grid">
            ...
            <article className="result-panel wide"><small className="kicker">EXPERIENCE</small>{analysis.experience.length ? analysis.experience.map((item, index) => <div className="improvement" key={`${item.company}-${item.role}-${index}`}><div><b>{item.role} · {item.company}</b><p>{item.duration}</p>{item.highlights.length > 0 && <ul>{item.highlights.map((highlight, highlightIndex) => <li key={`${index}-${highlightIndex}`}>{highlight}</li>)}</ul>}</div></div>) : <p>No work experience was confidently extracted.</p>}</article>
            <article className="result-panel wide"><small className="kicker">EDUCATION</small>{analysis.education.length ? analysis.education.map((item, index) => <div className="check-row" key={`${item.institution}-${item.degree}-${index}`}><b>{item.degree}</b><span>{item.year}</span><small>{item.institution}</small></div>) : <p>No education details were confidently extracted.</p>}</article>
