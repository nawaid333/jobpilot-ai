...
export default function AgentPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const [agentRes, actionsRes] = await Promise.all([fetch("/api/agent", { cache: "no-store" }), fetch("/api/agent/actions", { cache: "no-store" })]);
  ...
  return <main className="page-shell">
    <section className="page-hero"><div><p className="eyebrow">JOBPILOT AGENT</p><h1>Next best actions for your job search.</h1><p className="muted">One queue for applications, recruiting emails, follow-ups, interviews, and preparation.</p></div><button className="secondary-button" onClick={load} disabled={!!busy}>Refresh queue</button></section>
    ...
    {data && <>
      <section className="stats-grid"><div className="stat-card"><span>Active applications</span><strong>{data.summary.activeApplications}</strong></div><div className="stat-card"><span>Needs attention</span><strong>{data.actions.length}</strong></div><div className="stat-card"><span>Prepared packages</span><strong>{data.summary.prepared}</strong></div><div className="stat-card"><span>Inbox signals</span><strong>{data.summary.inboxSignals}</strong></div></section>
      {selectedWorkflow && <section className="panel"><div className="section-heading"><div><p className="eyebrow">AGENT WORKFLOW</p><h2>{selected.company || "Recruiting signal"} · {selected.role || "Review"}</h2></div><button className="secondary-button" onClick={() => setSelected(null)}>Close</button></div><div className="job-tags">{selectedWorkflow.map(([label, state]: string[]) => <span key={label}>{state === "complete" ? "✓ " : state === "current" ? "→ " : ""}{label}</span>)}</div><p className="muted">JobPilot assists with the next step while review and final submission remain under your control.</p><div className="profile-actions">{selected.applicationId && <Link className="secondary-button" href={`/application/${encodeURIComponent(selected.applicationId)}`}>Open application</Link>}{selected.type === "tailor" && <button className="primary-button" disabled={busy===selected.id} onClick={() => execute(selected)}>{busy===selected.id ? "Preparing…" : "Start tailoring"}</button>}{selected.type !== "tailor" && <a className="primary-button" href={hrefFor(selected)}>Continue</a>}<button className="secondary-button" disabled={busy===selected.persistentId} onClick={() => updateAction(selected, "completed")}>✓ Complete</button><button className="secondary-button" disabled={busy===selected.persistentId} onClick={() => updateAction(selected, "dismissed")}>Dismiss</button></div></section>}
      <section className="panel"><div className="section-heading"><div><p className="eyebrow">ACTION QUEUE</p><h2>What should happen next</h2></div></div>{data.actions.length === 0 ? <div className="empty-state"><h3>You’re caught up.</h3><p>No high-priority recruiting actions are pending.</p></div> : <div className="action-list">{data.actions.map((action: any) => <article className="action-card" key={action.persistentId}><div className="action-priority">P{action.priority}</div><div className="action-content"><div className="action-title-row"><h3>{action.title}</h3><span>{action.role}</span></div><strong>{action.company}</strong><p>{action.reason}</p></div><button className="primary-button" onClick={() => setSelected(action)}>{action.type === "tailor" ? "Start" : "View workflow"}</button></article>)}</div>}</section>
